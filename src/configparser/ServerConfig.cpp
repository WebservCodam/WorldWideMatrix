...
ServerConfig::ServerConfig(const std::string& serverName,
	...
	: _serverName(serverName), _listenDirectives(listenDirectives), _maxBodySize(maxBodySize), _errorPages(errorPages), _locations(locations), _keepaliveTimeout(keepaliveTimeout)
{
}
